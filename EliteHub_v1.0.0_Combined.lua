...
-- ║   ╚══════╝╚══════╝╚═╝   ╚═╝   ╚══════╝                           ║
-- ║                                                                   ║
-- ║   Version  : v1.0.0  (Full Combined Edition)                     ║
...
local function saveConfig(S)
    pcall(function()
        if HAS_WRITEFILE then
            ...
            if isfolder and isfolder("Elite Hub Scripts/Blox Fruits") then
                writefile("Elite Hub Scripts/Blox Fruits/"..LP.Name..".json",
                    HttpService:JSONEncode({Configs=_G.Settings.Configs,Misc=_G.Settings.Misc,HUD=_G.Settings.HUD}))
            end
        end
    end)
end
...
LP.CharacterAdded:Connect(function() STATS.deaths=STATS.deaths+1 end)
...
local function bypassTP(Point)
    ...
    root.CFrame = Point * CFrame.new(0,50,0); wait(0.2)
    root.CFrame = Point; wait(0.1)
    root.CFrame = Point * CFrame.new(0,50,0)
    root.Anchored = true; wait(0.1)
    root.CFrame = Point; wait(0.5)
    root.Anchored = false
...
local function toTarget(targetCF)
    ...
    if hum and hum.Health == 0 then
        repeat wait() until (getHum() and getHum().Health > 0); wait(0.2)
    end
...
-- Get nearest spawn island name (for legacy quest system)
local function GetIsLand(targetPos)
    local pos = type(targetPos)=="userdata" and targetPos.Position or targetPos
    local best, bestDist = nil, math.huge
    pcall(function()
        local team = tostring(LP.Team)
        local spawns = workspace._WorldOrigin.PlayerSpawns:FindFirstChild(team)
        if spawns then
            for _, v in pairs(spawns:GetChildren()) do
                local d = (pos - v:GetModelCFrame().p).Magnitude
                if d < bestDist then bestDist=d; best=v.Name end
            end
        end
    end)
    return best
...
-- ════════════════════════════════════════════
local _QC_cache, _QC_lastLv = nil, -1
local function QuestCheck()
    local Lvl = getLevel()
    if Lvl == _QC_lastLv and _QC_cache then return _QC_cache end
    _QC_lastLv = Lvl
...
-- Fast attack loop (CombatFramework continuous attack while farming)
coroutine.wrap(function()
    while task.wait(0.1) do
        pcall(function()
            ...
            if t == "Normal"  then if tick()-cooldownfastattack > 0.9  then wait(0.1)  cooldownfastattack=tick() end
            elseif t == "Fast" then if tick()-cooldownfastattack > 1.5  then wait(0.01) cooldownfastattack=tick() end
            elseif t == "Slow" then if tick()-cooldownfastattack > 0.3  then wait(0.7)  cooldownfastattack=tick() end end
            bladeAttack()
        end)
    end
end)()
    ...
    for _, obj in pairs(workspace:GetDescendants()) do
        if obj:IsA("Model") and obj~=getChar() and not Players:GetPlayerFromCharacter(obj) then
            ...
            if hum and hrp and hum.Health>0 then
                ...
                if n2==nl or n2:find(nl,1,true) or nl:find(n2,1,true) then
...
local function findAllMobs(radius)
    ...
    for _, obj in pairs(workspace:GetDescendants()) do
        if obj:IsA("Model") and obj~=getChar() and not Players:GetPlayerFromCharacter(obj) then
            ...
            if hum and hrp and hum.Health>0 then
                local d = (hrp.Position-root.Position).Magnitude
                if d<(radius or 25) then table.insert(found,{obj=obj,hrp=hrp,hum=hum,dist=d}) end
            end
        end
    end
...
-- ════════════════════════════════════════════
--  MULTI-METHOD COMBAT ENGINE  (6 methods)
-- ════════════════════════════════════════════
local function attackMob(mob)
    ...
    PosMon = hrp.CFrame  -- update BringMob position
    task.wait(0.04)

    -- 2. Blade attack (CombatFramework)
    if _CF_ok then bladeAttack() end

    -- 3. firetouchinterest on all mob parts
    if HAS_FTI then
        ...
        for _, part in pairs(mob:GetChildren()) do
            if part:IsA("BasePart") then
                pcall(firetouchinterest, part, root, 0); task.wait(0.01)
                pcall(firetouchinterest, part, root, 1)
            end
        end
    end

    -- 4. Tool handle touch + tool remotes
    if c then
            ...
            if tool:IsA("Tool") then
                ...
                if handle and HAS_FTI then
                    for _, part in pairs(mob:GetDescendants()) do
                        if part:IsA("BasePart") then
                            pcall(firetouchinterest, part, handle, 0); task.wait(0.01)
    ...
    -- 5. ReplicatedStorage damage remotes
    ...
    -- 6. ClickDetector + health drain fallback
    if HAS_FCD then
    ...
    if killed then
        STATS.kills=STATS.kills+1; STATS.kpmBucket=STATS.kpmBucket+1
        if QUEST.state=="farming" then
            QUEST.killsDone=QUEST.killsDone+1; STATS.questKills=STATS.questKills+1
        end
    end
    return killed
end
...
local function buildWatermark()
    ...
    RunService.Heartbeat:Connect(wrap(function()
        ...
        if sg.Enabled then
            lbl.Text = "  Elite Hub v1.0.0  |  FPS: "..FPS.."  |  "..STATUS
        end
    end))
end
...
local function showLoader()
    ...
    step(0.50,"Arming combat engine...",        T.Red,   "[ OK ] 6-method attacker + blade-hit system")
        ...
        if S.MobESP then
            for _,obj in pairs(workspace:GetDescendants()) do
                if obj:IsA("Model") and not Players:GetPlayerFromCharacter(obj) then
                    local hum=obj:FindFirstChildOfClass("Humanoid"); local hrp=obj:FindFirstChild("HumanoidRootPart")
                    if hum and hrp and hum.Health>0 then mkBill(hrp,obj.Name,Color3.fromRGB(255,170,60)) end
                end
            end
        end
    end
...
task.spawn(function()
    while task.wait(5) do
        ...
        if S.FOV then Camera.FieldOfView=S.FOV end
    end
end)
...
task.spawn(function()
    ...
    while task.wait(0.5) do
        ...
        if QUEST.state=="idle" then
            if qData then
                QUEST.mob = {name=qData[6], pos=qData[2] and qData[2].Position or mobData.pos, questPos=qData[2] and qData[2].Position, kills=10}
                QUEST.killsNeeded = 10; QUEST.killsDone = 0
            else
    ...
    while task.wait(3) do
        ...
        if S.AutoEatFruit then
            ...
            pcall(function()
                if f.part and f.part.Parent then
                    if HAS_FCD then
                        local cd=f.part:FindFirstChildOfClass("ClickDetector") or f.part.Parent:FindFirstChildOfClass("ClickDetector")
