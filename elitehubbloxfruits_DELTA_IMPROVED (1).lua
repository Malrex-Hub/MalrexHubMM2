...
--// cleaned malformed obfuscation artifacts
--// normalized callbacks
--// patched notification strings
--// reduced executor conflicts

-- EliteHub v1.0.0 | Premium Blox Fruits Hub
    ...
    if _execName:find(banned, 1, true) then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title    = "[BAN]  EliteHub -- Executor Banned",
...
function Bypass.StartAntiKick()
    ...
    pcall(function()
        ...
        LP.Kick = function(self, msg)
            ...
            Notif("[WARN] Anti-Kick", "Kick blocked: " .. tostring(msg or ""), 6)
...
function Bypass.SafeMode()
    ...
    Notif("[SHLD] SAFE MODE", "All dangerous features disabled!", 5)
end
...
local MATRIX_CHARS = "01AIUEOKKKKKSSSSSTTTTTNNNNN"
local matrixCols = {}
            ...
            for _, p in ipairs(Players:GetPlayers()) do
                if p ~= LP and p.Character then
                    ...
                    if r and h then
                        ...
                        local txt = p.Name .. "  " .. math.floor(h.Health) .. "<3  " .. dist .. "m"
                        if ESPObj[key] then
                            pcall(function()
                    ...
                    if h and r and h.Health > 0 and not Players:GetPlayerFromCharacter(obj) then
                        ...
                        if not ESPObj[key] then
                            local bb = MakeBillboard(r,
                                obj.Name .. "  " .. math.floor(h.Health) .. "<3",
            ...
            for _, bname in ipairs(BossNames) do
                ...
                if boss and boss:IsA("Model") then
                    ...
                    if h and r and h.Health > 0 then
                        ...
                        if not ESPObj[key] then
                            local bb = MakeBillboard(r,
                                "[WARN] "..bname.."  "..math.floor(h.Health).."/"..math.floor(h.MaxHealth),
            ...
            for _, obj in ipairs(Workspace:GetDescendants()) do
                if obj:IsA("Model") and (obj.Name:find("Fruit") or obj:FindFirstChild("Fruit")) then
                    ...
                    if part then
                        ...
                        if not ESPObj[key] then
                            ESPObj[key] = MakeBillboard(part, "[fruit] "..obj.Name,
                        ...
                        if r and h and h.Health > 0 then
                            ...
                            if not ESPObj[key] then
                                ESPObj[key] = MakeBillboard(r,
                                    "~ Sea Beast  "..math.floor(h.Health).."<3",
            ...
            for _, obj in ipairs(Workspace:GetDescendants()) do
                if obj:IsA("Model") and (obj.Name:find("Fruit") or obj:FindFirstChild("Fruit")) then
                    ...
                    if not seen[key] then
                        seen[key] = true
                        Notif("[fruit] Fruit Spawned!", obj.Name .. " appeared on the map!", 6)
                    end
                end
            end
        end))
    ...
    onOn = function()
        ...
        Conn("fruit_spawn_timer", RunService.Heartbeat:Connect(function()
            ...
            if tick()-lastSpawnCheck > spawnInterval*0.95 then
                Notif("[fruit] Spawn Timer", "A Devil Fruit may spawn soon!", 6)
                lastSpawnCheck = tick()
            end
        end))
    end,
...
Feat("spin_lock", {
    ...
    onOn = function()
        ...
        if not ok then
            ...
            -- spin_lock_restore: hook active, will be cleaned by Drop("spin_lock") on disable
        end
    end,
    onOff = function()
...
Feat("ac_safemode", {
    name="[SHLD] Safe Mode", cat="Bypass", danger="safe",
...
local function BuildDashboard()
    ...
    NewLabel({
        Size = UDim2.new(1,0,1,0), Text = "OK  KEYLESS",
...
local TabDefs = {
    ...
    {id="Combat",       label="Combat",       icon="X", order=2},
    {id="AutoFarm",     label="Auto Farm",    icon="O", order=3},
    {id="ESP",          label="ESP",          icon="O", order=4},
    {id="Teleports",    label="Teleports",    icon="^", order=5},
    {id="Movement",     label="Movement",     icon="->", order=6},
    {id="Visuals",      label="Visuals",      icon="O", order=7},
    ...
    {id="Bypass",       label="Bypass",       icon="[SHLD]", order=11},
    {id="AntiBan",      label="Anti-Ban",     icon="[shld]", order=12},
    ...
    conn = RunService.Heartbeat:Connect(function(dt)
        ...
        if newIdx ~= msgIdx and newIdx <= #StatusMessages then
            ...
            if checkIdx <= #CheckLabels and CheckLabels[checkIdx] then
                CheckLabels[checkIdx].Text = "OK  " .. CheckItems[checkIdx]
...
task.spawn(function()
    ...
    Notif("OK  Elite Hub Loaded",
        "v"..HUB_VERSION.." • "..cnt.." features  .  Keyless", 5)
    task.wait(1.2)
    Notif("Executor",
        tostring(EliteHub_Executor or "unknown"):upper()..
        " • Unsupported executors blocked", 4)
    task.wait(1.2)
    Notif("Controls",
...
--// IMPROVEMENTS PATCH V1.0.1

local TweenService = game:GetService("TweenService")
local UserInputService = game:GetService("UserInputService")

-- smoother notifications
function SmoothNotify(title,text)
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification",{
            Title = title,
            Text = text,
            Duration = 5,
            Icon = "rbxassetid://11395204805"
        })
    end)
end

SmoothNotify("ELITE HUB","delta optimized build loaded")

-- anti duplicate ui protection
pcall(function()
    local cg = game:GetService("CoreGui")
    local count = 0

    for _,v in pairs(cg:GetChildren()) do
        if tostring(v.Name):lower():find("elite") then
            count += 1
...
-- cleaner tween helper
function TweenObject(obj,props,time)
    TweenService:Create(
        obj,
...
SmoothNotify("Sea Detector",GetCurrentSea())
